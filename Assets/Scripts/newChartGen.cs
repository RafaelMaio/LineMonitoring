...
public class newChartGen : MonoBehaviour
{
    ...
    string base_url = "http://192.168.1.159:5500/station/chartgen.png?chart=";
    ...
    public void GetSample(string station, string attr, int index, GameObject go, bool isTall = false)
    {
        getchartfromurl(base_url.Replace("station", station).Replace("address", Getaddress()) + $"specSamples&attr={attr}&index={index}" + (isTall ? "&size=tall" : ""), go, isTall);
    }
