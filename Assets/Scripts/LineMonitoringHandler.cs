    ...
    private void Start()
    {
        ...
        foreach (GameObject go in allObjects)
        {
            ...
            else if (go.GetComponent<PinchSlider>() != null)
            {
                ...
                go.GetComponent<PinchSlider>().OnInteractionEnded.AddListener(delegate { usabilityTest.writeEvent(staticFunctions.getPathName(go)); });
            }
        }
    }
    ...
    public void filterBy(string by)
    {
        ...
        for (int i = 0; i < contentStations.transform.childCount; i++)
        {
            ...
            if (by.Equals("Cycle Times"))
            {
                ...
                newChartG.GetSample(station_number, "totalCycleTime", 10, staticFunctions.FindChildByRecursion(spawnedStation.transform, "VegaLiteIntegration").gameObject, true);
            }
            else if (by.Equals("KPIs"))
            {
                ...
                newChartG.GetSample(station_number, "fpy", 10, staticFunctions.FindChildByRecursion(spawnedStation.transform, "VegaLiteIntegration").gameObject, true);
            }
            else
            {
