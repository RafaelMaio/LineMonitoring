...
public struct CycleTimes
{
    ...
    public CycleTimes(string id, string line, string station, string serialNumber, string reference, string timeStamp,
        float ct, float et, float pt, float wt)
    {
        this.id = id;
        this.line = line;
...
public struct KPIs
{
    ...
    public KPIs(float shiftIdentifier, string locationId, string validFrom, string validTo, float oee, float oeeSetpoint, float availability, 
                float efficiency, float quality, float fpy, float fpySetPoint, float partCount, float partCountSetPoint, float countNIO, float productivity)
    {
...
public class ValuesHandler : MonoBehaviour
{
    ...
    { 
        ...
        string urlKPIs = "https://ews-emea.api.bosch.com/it/application/api/augmanity-pps4-dummy/d/v1/api/kpi/current-shift";
        ...
        string urlBottleneck = "https://ews-emea.api.bosch.com/it/application/api/augmanity-pps4-dummy/d/v1/api/bottlenecks/actual";
    ...
    /// <returns></returns>
    public int checkThereshold(int station, string varName, bool cycle_or_kpi, bool positive)
    {
        if (cycle_or_kpi)
        {
            if (positive)
            {
                float red = (float)allStations.targetCycles[station].GetType().GetField(varName).GetValue(allStations.targetCycles[station]) * 1.1f;
                float yellow = (float)allStations.targetCycles[station].GetType().GetField(varName).GetValue(allStations.targetCycles[station]) * 1.05f;
                if ((float)allStations.cycles[station].GetType().GetField(varName).GetValue(allStations.cycles[station]) > red) return 2;
                else if ((float)allStations.cycles[station].GetType().GetField(varName).GetValue(allStations.cycles[station]) > yellow) return 1;
                else return 0;
            }
            else
            {
                float red = (float)allStations.targetCycles[station].GetType().GetField(varName).GetValue(allStations.targetCycles[station]) * 0.9f;
                float yellow = (float)allStations.targetCycles[station].GetType().GetField(varName).GetValue(allStations.targetCycles[station]) * 0.95f;
                if ((float)allStations.cycles[station].GetType().GetField(varName).GetValue(allStations.cycles[station]) < red) return 2;
                else if ((float)allStations.cycles[station].GetType().GetField(varName).GetValue(allStations.cycles[station]) < yellow) return 1;
                else return 0;
            }
        ...
        else
        {
            if (positive)
            {
                float red = (float)allStations.targetKPIs[station].GetType().GetField(varName).GetValue(allStations.targetKPIs[station]) * 1.1f;
                float yellow = (float)allStations.targetKPIs[station].GetType().GetField(varName).GetValue(allStations.targetKPIs[station]) * 1.05f;
                if ((float)allStations.kpis[station].GetType().GetField(varName).GetValue(allStations.kpis[station]) > red) return 2;
                else if ((float)allStations.kpis[station].GetType().GetField(varName).GetValue(allStations.kpis[station]) > yellow) return 1;
                else return 0;
            }
            else
            {
                float red = (float)allStations.targetKPIs[station].GetType().GetField(varName).GetValue(allStations.targetKPIs[station]) * 0.9f;
                float yellow = (float)allStations.targetKPIs[station].GetType().GetField(varName).GetValue(allStations.targetKPIs[station]) * 0.95f;
                if ((float)allStations.kpis[station].GetType().GetField(varName).GetValue(allStations.kpis[station]) < red) return 2;
                else if ((float)allStations.kpis[station].GetType().GetField(varName).GetValue(allStations.kpis[station]) < yellow) return 1;
                else return 0;
            }
